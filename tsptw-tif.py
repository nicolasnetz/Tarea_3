...
class Instancia:
    ...
    def leer_archivo(self,instance):
        ...
        self.W = {i: [i for i in range(self.a[i], self.b[i] + 1)] for i in self.V}
        self.p = self.V[0]
        self.q = self.V[0]
...
class ModeloTimeIndexed():
    ...
    def build(self,ins):
        ...
        self.y = self.mdl.binary_var_dict( [(i,j,t) for i,j in ins.arcos for t in ins.W[i]], name = 'y')
        ...
        for i in ins.V:
            self.mdl.add_constraint(self.mdl.sum(self.z[(i,t)] for t in ins.W[i]) == 1 , ctname= 'restriccion2: {}'.format(i))
        ...
        [tau for tau in ins.W[k] if tau <= ins.a[i] - ins.c[k,i] ]
        
        # Restriccion 4:
        for i in ins.V:
            if i!=ins.p:
                for t in ins.W[i]:
                    self.mdl.add_constraint(self.mdl.sum(self.y[k,i,tau] for k in ins.V if k!=i for tau in 
                    []
                    ) == self.z[i,t], ctname= 'restriccion4: z_{}'.format(i))
